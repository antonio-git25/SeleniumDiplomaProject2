...
class Rifled_patron_page(Base):
    ...
    typebullet_down = "//span[contains(text(),'Тип пуль')]"
    typebullet_input = "//input[@id='tip_pul-filter']"
    typebullet_click = "//label[contains(text(),'Fmj')]"

    patron = "//button[@title='Купить Патрон нарезной ТЕХКРИМ калибр 308 WIN Купра 12,8 гр. тяж.пуля (в пачке 10 шт.) (в кор.330)']"
    ...
    def get_typebullet_input(self):
        return WebDriverWait(self.driver, 30).until(EC.element_to_be_clickable((By.XPATH, self.typebullet_input)))
    ...
    def set_type_bullet(self):
        ...
        time.sleep(1)
        self.get_typebullet_input().send_keys("Fmj")
        time.sleep(1)
        self.get_typebullet_click().click()
        time.sleep(1)
        print("Type of bullet: Fmj")

    def click_patron(self):
        self.get_patron().click()
        print("Choose bullet: Патрон нарезной ТЕХКРИМ калибр 308 WIN Купра 12,8 гр. тяж.пуля")
        time.sleep(10)
    ...
    def choose_patron(self):
        ...
        self.click_patron()
        time.sleep(10)
        self.get_screenshot("rifled-patron", "catalog")
