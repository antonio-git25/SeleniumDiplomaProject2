...
def test_buy_rifled():
    ...
    print("Start Test 2: Покупка нарезной винтовки")
    ...
    fp.select_rifled_gun()

    rp = Rifled_page(driver)
    rp.check_current_location()
    rp.set_product_filters()
    rp.choose_product()
    rp.click_basket()
    ...
    bp.make_screenshot("rifled-gun", "basket")
    bp.confirmation()

    op = Order_page(driver)
    op.check_current_location()
    op.set_personal_data()
    op.make_screenshot("rifled-gun", "persondata")
    op.go_next()
    op.set_delivery()
    op.make_screenshot("rifled-gun", "delivery")
    op.go_next()
    op.show_final_order()
    ...
    print("Complete Test 2")
    time.sleep(5)
    driver.close()


#python -m pytest -s -v test_buy_rifled.py