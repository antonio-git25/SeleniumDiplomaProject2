...
def test_buy_smoothbore():
    ...
    print("Start Test 1: покупка гладкоствольного карабина")
    ...
    fp.select_smoothbore_gun()

    sp = Smoothbore_page(driver)
    sp.check_current_location()
    sp.set_product_filters()
    sp.choose_product()
    sp.click_basket()
    ...
    bp.make_screenshot("smoothbore-gun", "basket")
    bp.confirmation()

    op = Order_page(driver)
    op.check_current_location()
    op.set_personal_data()
    op.make_screenshot("smoothbore-gun", "persondata")
    op.go_next()
    op.set_delivery()
    op.make_screenshot("smoothbore-gun", "delivery")
    op.go_next()
    op.show_final_order()
    ...
    print("Complete Test 1")
    time.sleep(5)
    driver.close()


#python -m pytest -s -v test_buy_smoothbore.py