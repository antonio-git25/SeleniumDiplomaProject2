...
from pages.traumatic_patron_page import Traumatic_patron_page
...
def test_buy_traumatic_patron():
    ...
    print("Start Test 6: Покупка патронов для ОООП 10х28")
    ...
    ap.select_traumatic_patron()

    tpp = Traumatic_patron_page(driver)
    tpp.check_current_location()
    tpp.set_product_filters()
    tpp.choose_patron()
    tpp.click_basket()
    ...
    bp.stuff_value()
    bp.make_screenshot("traumatic-patron", "basket")
    bp.confirmation()

    op = Order_page(driver)
    op.check_current_location()
    op.set_personal_data()
    op.make_screenshot("traumatic-patron", "persondata")
    op.go_next()
    time.sleep(1)
    op.set_delivery()
    op.make_screenshot("traumatic-patron", "delivery")
    op.go_next()
    op.show_final_order()
    ...
    print("Complete Test 6")
    time.sleep(5)
    driver.close()


#python -m pytest -s -v test_buy_traumatic_patron.py